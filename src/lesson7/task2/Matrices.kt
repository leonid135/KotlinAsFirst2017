...
fun generateSpiral(height: Int, width: Int): Matrix<Int> {
    ...
    while (value <= height * width) {
        ...
        for (j in k - 1 until width - k + 1) {
            ...
            result[Cell(k - 1, j)] = value++
        }
        for (j in k until height - k + 1) {
            if (value > height * width) break
            result[Cell(j, width - k)] = value++
        }
        for (j in width - k - 1 downTo k - 1) {
            if (value > height * width) break
            result[Cell(height - k, j)] = value++
        }
        for (j in height - k - 1 downTo k) {
            if (value > height * width) break
            result[Cell(j, k - 1)] = value++
        }
    }
    return result
}
...
fun generateRectangles(height: Int, width: Int): Matrix<Int> {
    ...
    while (value <= height * width) {
        ...
        for (j in k until height - k + 1) {
            if (value > height * width) break
            result[Cell(j, width - k)] = k
            value++
        }
        for (j in width - k - 1 downTo k - 1) {
            if (value > height * width) break
            result[Cell(height - k, j)] = k
            value++
        }
        for (j in height - k - 1 downTo k) {
            if (value > height * width) break
...
fun <E> rotate(matrix: Matrix<E>): Matrix<E> {
    ...
    for (i in 0 until matrix.width) {
        for (j in 0 until matrix.width) {
            result[Cell(j, matrix.width - i - 1)] = matrix[Cell(i, j)]
        }
    }
    return result
}
